...
class CRUDBase(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    async def get(self, db: AsyncSession, id: Any) -> Optional[ModelType]:
        _ = await db.execute(select(self.model).where(self.model.id == id))
        return _.scalar()

    async def get_multi_filter(self, db: AsyncSession, *, filters: FilterType, skip: int = 0, limit: int = 100) -> List[ModelType]:
    ...
    async def get_multi(
        ...
        _ = await db.execute(select(self.model).offset(skip).limit(limit).order_by(desc(self.model.id)))
        return _.scalars().all()
